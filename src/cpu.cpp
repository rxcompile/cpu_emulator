...
std::optional<Flags> lookup(uint8_t p, Code instruction)
{
    using namespace Flag;
    switch (instruction)
    {
...
auto microCode(uint8_t p, Code instruction)
{
    return p < 2 ? preamble(p) : lookup(p - 2, instruction);
}
}

bool Computer::isHalted() const
{
    return flags.test(Flag::HALT);
}
...
{
    if (const auto mc = microCode(mip, static_cast<Code>(hi(instruction))))
    {
        flags = *mc;
        ++mip;
    }
